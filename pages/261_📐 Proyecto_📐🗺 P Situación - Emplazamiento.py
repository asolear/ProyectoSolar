...
class contenido:
    ...
    def contenido(fig):
        contenido.provincia(fig)
        contenido.pais(fig)
        contenido.barrio(fig)
        contenido.satelite(fig)
        contenido.qr(fig)
    ...
    def pais(fig):
        ax = fig.add_axes([.1, .55, 0.3, 0.3], frameon=False)
        #
        oo = (s.s['FV']['inputs']['Ubicacion']['_lng'],
              s.s['FV']['inputs']['Ubicacion']['_lat'])
        zoom = 4.0
        geometry = [
            Point(oo).buffer(zoom),
            Point(oo).buffer(zoom/20),

        ]
        db = gpd.GeoDataFrame(crs='epsg:4326', geometry=geometry)
        db.boundary.plot(ax=ax, color='r', linewidth=.5, alpha=.95)
    ...
    def provincia(fig):
        ax = fig.add_axes([.5, .55, 0.3, 0.3], frameon=False)
        oo = (s.s['FV']['inputs']['Ubicacion']['_lng'],
              s.s['FV']['inputs']['Ubicacion']['_lat'])
        zoom = .2
        geometry = [
    ...
    def barrio(fig):
        ax = fig.add_axes([.1, .15, 0.3, 0.3], frameon=False)
        oo = (s.s['FV']['inputs']['Ubicacion']['_lng'],
              s.s['FV']['inputs']['Ubicacion']['_lat'])
        zoom = .01
        geometry = [
    ...
    def satelite(fig):
        ax = fig.add_axes([.5, .15, 0.3, 0.3], frameon=False)
        oo = (s.s['FV']['inputs']['Ubicacion']['_lng'],
              s.s['FV']['inputs']['Ubicacion']['_lat'])
        zoom = .0005
        geometry = [
            Point(oo).buffer(zoom),
            # Point(oo).buffer(zoom/10),
